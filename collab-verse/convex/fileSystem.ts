    ...
    handler: async (ctx, args) => {
        ...
        if (args.parentId !== undefined) {
            if (args.parentId === null) {
                ...
                pId = ctx.db.normalizeId("filesystem", args.parentId) ?? undefined;
                if (!pId) return { success: false, error: "Invalid parent ID" };
            }
        }
...
const updateFileContent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.executionTime !== undefined) updateData.executionTime = args.executionTime;
        if (args.isExecutable !== undefined) updateData.isExecutable = args.isExecutable;
...
export const saveFileContent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (file.type !== "file") return { success: false, error: "Can only save content for files" };
        ...
        if (existingContent) {
            ...
            return { success: true, contentId: existingContent._id, action: "updated" };
        } else {
            // Create new content
            const contentId = await ctx.db.insert("fileContent", {
                fileId: id,
